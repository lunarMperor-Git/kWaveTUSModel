function signal = new_toneBurst(sample_freq, signal_freq, PRF, num_cycles, duty_cycle, varargin)
...
%     NEW: PRF                 - pulse repetition frequency
%     REVISED: num_cycles      - number of seconds of sonication, NOT number of sinusoidal oscillations
%     NEW: duty_cycle          - percentage of time with transducer on
%
% OPTIONAL INPUTS:
...
dt = 1 / sample_freq; % time step, kgrid.dt = 8.2e-8 seconds

% every 1 second, 10 burst as PRF = 10
% sample_freq = Nt_per_second

Nt_per_period = sample_freq / PRF; % time steps per burst + cooling

% create the tone burst
% tone_length = num_cycles / (signal_freq);
tone_length = ceil((duty_cycle / 100) * Nt_per_period); % time steps per burst
cool_length = Nt_per_period - tone_length;

tone_burst_duration = tone_length * dt; % burst duration in sec

num_cycles_per_burst = tone_burst_duration * signal_freq; % # of signal cycles per burst

tone_t = 0:dt:(tone_burst_duration - dt); % burst time array
cool_t = zeros(1, cool_length); % cool time array
...
if numel(envelope) == 2
    ...
    if num_cycles_per_burst < (num_ring_up_cycles + num_ring_down_cycles)
...
tone_burst = [tone_burst, cool_t];
tone_burst = repmat(tone_burst, 1, PRF * num_cycles);
