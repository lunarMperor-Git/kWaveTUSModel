...
o	Windows (CMD): charm subject_id T1_file_path T2_file_path  --forcerun
o	macOS: charm subject_id T1_file_path T2_file_path
o	NOTE: subject_id = any name you want to assign, T2 file path is optional.
4.	In 3D Slicer, plot out focal position. Following BabelBrain’s planning guide may help.
o	In the model, you must input the Cartesian coordinates (x, y, z)!
...
o	NOTE: cortical/trabecular segmentation is artificial and does not follow SimNIBS segmentation, which aligns with BabelBrain
...
Outputs: Max Isppa in brain, Isppa at target, max Ispta in brain, Ispta at target, max temperature at target, brain, skin, and skull, offset of focus from the target, and FWHM size
